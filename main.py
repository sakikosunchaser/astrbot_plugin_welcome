...
class WelcomePlugin(Star):
    ...
    def _build_chain(self, text: str, image: str = ""):
        chain = []
        if image:
    ...
    async def _send_welcome_active(self, group_id: str, text: str, image: str = ""):
        image = self._normalize_image(image)
        ...
        await self._send_group_message(group_id, fallback)
    ...
    async def _handle_welcome(self, event: Any):
        ...
        await self._send_welcome_active(group_id=group_id, text=text, image=image)
